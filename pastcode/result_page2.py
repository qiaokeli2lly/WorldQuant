...
from backtest import run_backtest, calculate_metrics, multi_strategy_backtest, grid_search_ma, calc_outperform_stats
...
def show():
    ...
    with st.spinner("正在加载数据..."):
        ...
        with st.expander("⚙️ 当前策略参数详解"):
            if strategy == "双均线":
                st.markdown(f"- 短期均线: {params['short_ma']}日\n- 长期均线: {params['long_ma']}日")
            elif strategy == "RSI策略":
                st.markdown(f"- RSI周期: {params['period']}\n- 买入阈值: RSI < {params['oversold']}\n- 卖出阈值: RSI > {params['overbought']}")
            else:
                st.markdown(f"- 布林带周期: {params['period']}\n- 标准差倍数: {params['std_dev']}")
