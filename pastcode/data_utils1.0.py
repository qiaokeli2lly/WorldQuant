...
# 配置请求间隔（秒）
REQUEST_INTERVAL = (0.5, 1.5)      # 每次请求前随机休眠区间
...
def _fetch_data_with_fallback(symbol, start_date, end_date, max_retries):
    ...
    for attempt in range(max_retries):
        ...
        if attempt < max_retries - 1:
            ...
            print(f"baostock 第 {attempt+1} 次失败，等待 {wait:.1f} 秒后重试...")
            time.sleep(wait)
...
def _fetch_from_baostock(symbol, start_date, end_date):
    """使用 baostock 获取数据（请求前随机延时，单日周末直接跳过）"""
    # 单日周末判断
