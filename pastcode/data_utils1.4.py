...
# ---------- 数据源获取函数（已修改 baostock 支持重试）----------
def _fetch_data_with_fallback(symbol, start_date, end_date, max_retries):
    # 先尝试 baostock（内部支持重试）
    df = _fetch_from_baostock(symbol, start_date, end_date, max_retries)
    if not df.empty:
...
def _fetch_from_baostock(symbol, start_date, end_date, max_retries):
    """
    从 baostock 拉取数据，支持重试 max_retries 次
    """
    ...
    for attempt in range(max_retries):
        try:
            ...
            bs.logout()
            if not data:
                print(f"baostock 返回空数据，尝试 {attempt+1}/{max_retries}")
                if attempt < max_retries - 1:
                    wait_time = (2 ** attempt) + random.uniform(0, 1)
                    time.sleep(wait_time)
                continue
            ...
            print(f"baostock 第 {attempt+1} 次失败: {e}")
            if attempt < max_retries - 1:
                wait_time = (2 ** attempt) + random.uniform(0, 1)
                print(f"等待 {wait_time:.1f} 秒后重试...")
                time.sleep(wait_time)
            else:
                print("baostock 所有重试均失败")
    return pd.DataFrame()
