...
def calculate_metrics(df, risk_free_rate=0.03):
    ...
    if 'signal' in df.columns:
        ...
        trade_cnt = 0
    # 最终资产
    if 'strategy_value' in df.columns:
        final_asset = df['strategy_value'].iloc[-1]
        final_bench = df['benchmark_value'].iloc[-1]
    else:
        final_asset = strategy_net.iloc[-1] * 100000
        final_bench = benchmark_net.iloc[-1] * 100000
    metrics = {
