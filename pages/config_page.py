...
def show():
    ...
    if 'config' in st.session_state:
        ...
        default_end = cfg.get('end_date', pd.to_datetime("2026-05-31"))
    ...
    # 热门股票（折叠）
    with st.expander("🔥 热门股票（点击展开）", expanded=False):
        popular = {
    ...
    with st.expander("📖 参数说明与技巧（点击展开）", expanded=False):
        ...
        - **RSI周期/阈值**：周期短敏感，周期长平滑。阈值在牛市中可放宽。  
        - **布林带倍数**：倍数越大，通道越宽，触及轨道的概率越低。  
        ### 🛡️ 过滤条件
        ...
        - **调仓频率**：越高交易成本越大，但能更快适应市场。  
        - **手续费/滑点**：实盘必须设置，否则回测结果偏乐观。  
        """)
    ...
    if strategy == "双均线":
        st.info("📈 **双均线**：适合有明显涨跌趋势的股票。金叉买，死叉卖。震荡市假信号多。")
    elif strategy == "RSI策略":
        st.info("📊 **RSI策略**：适合横盘震荡的股票。超卖买，超买卖。单边牛市会过早卖出。")
    else:
        st.info("📐 **布林带**：适合波动规律的股票。下轨买，上轨卖。突破轨道时可能反复止损。")
    ...
    with st.form("config_form"):
        ...
        # 副图指标（折叠）
        with st.expander("📉 副图指标", expanded=False):
        ...
        # 回测参数（折叠）
        with st.expander("⚙️ 回测参数", expanded=False):
            initial_capital = st.number_input("初始资金(元)", value=default_initial_capital, step=10000)
            rebalance_freq = st.selectbox("调仓频率", ["每日", "每周", "每月"],
                                          index=["每日","每周","每月"].index(
                                              {'D':'每日','W':'每周','M':'每月'}[default_rebalance_freq]))
        ...
        # 策略优化选项（折叠）
        with st.expander("🛡️ 策略优化选项", expanded=False):
